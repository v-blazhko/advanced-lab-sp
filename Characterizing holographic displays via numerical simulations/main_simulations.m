...
if z_p==0
    ...
    switch method
        case 'Fresnel'
            % ADD YOUR CODE HERE
            k = 2*pi / lambda;
            z_p = z_c - z_p; % note in the istructions
            obj_in = a_p * ( (exp(1j*k*z_p)) / (sqrt(1j*lambda*z_p)) * ( 1j*k*(((x-x_p).^2)/(2*z_p)) + phi_p) );
        ...
        case 'RS'
            ...
            z_p = z_c - z_p;
            x2 = (x-x_p).^2;
            obj_in = a_p * (z_p ./ (sqrt(1j*lambda) * (x2 + z_p^2).^0.75)) .* exp (1j*k*sqrt(x2 + z_p^2)+phi_p);
